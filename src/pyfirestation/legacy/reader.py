...
def wind_parser(origin: Union[str, PathLike],
    ...
    with origin_handle(origin, mode=mode) as f:
        ...
        _ = f.readline()
        # 首先读取剩余的所有行，然后将这些行的顺序反转，最后对这些行构成的矩阵进行转置。Read remaining lines. Rows are reversed and grid transposed..
        reader = csv.reader(f, delimiter=" ", skipinitialspace=True)
        grid_data = [[v for v in row if v] for row in reader if row]

    # 数据的第一行包含了用于构建三维数组（3D array）的维度信息。First line of data is the dimension of the 3D array
    nx, ny, nz = [int(i) for i in grid_data[0] if i]

    # 剩余的行包含每个单元格的坐标与风速，它们是交错排列的。Remaining lines include the coordinates and wind speed of each cell interleaved
    coord = grid_data[1:nx * ny * nz * 2 + 1:2]
    wind = grid_data[2:nx * ny * nz * 2 + 1:2]

    # Extract X and Y axis coordinates 从坐标数据coord中单独提出x和y轴坐标
    cx = [i[0] for i in coord[::ny * nz]]
    cy = [i[1] for i in coord[:ny * nz:nz]]

    # Extract Z coordinates (cell dependent)三维数组 wind_height_3d，其中每个元素是网格中每个单元格的Z坐标值。
    wind_height_3d = np.array([[[coord[i * ny * nz + j * nz + k][2]
                                 for k in range(nz)]
                                for j in range(ny)]
                               for i in range(nx)],
                              dtype=np.float_)[:, :, 1:3]
    # 计算每个单元格的风层高度差，计算沿Z轴方向的高度差。wind_height_data，它是一个二维数组，表示每个X-Y平面上的单元格在Z轴方向上第一和第二层之间的高度差。这个数组的形状是 (nx, ny)，与X-Y平面的维度相匹配，可以用于后续的风速数据处理或可视化。
    wind_height_data = np.diff(wind_height_3d, axis=2)[:, :, 0]

    # Extract 3D wind speed
    wind_layer = 1
    # wind_speed_data实际上是一个二维数组，表示在X-Y平面上每个单元格在选定风层上的风速数据。(nx, ny)
...
def wind_height_parser(origin: Union[str, PathLike],
                     *,
                     mode: Optional[DataInputMode] = None) -> Tuple['GridData', List[float], List[float]]:
    """Reads fuel distribution grid from a multi-line string or file.
    ...
    grid_data = GridData.from_EsriASCII(ascii_grid)
    return grid_data
