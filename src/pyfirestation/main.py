...
def main() -> None:
    ...
    print("Reading wind height data...")
    wind_height_data = wind_height_parser(files['wind_height'])

    # wind_speed_data = wind_height_parser(files['wind_speed'])

    wind_speed_data = np.full((299, 265, 3), 5)
    # wind_speed_data = GridData(data=wind_speed_data,
    #                       cx = wind_cx,
    #                       cy = wind_cy)
    ...
    wind_grid = WindGrid(speed_array=wind_speed_data,
                         height_array=wind_height_data.data,
                         cx=terrain_grid_data.cx,
                         cy=terrain_grid_data.cy)
